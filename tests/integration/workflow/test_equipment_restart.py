...
def request_for() -> OperationRequest:
    return OperationRequest(
        message="resume equipment repair after restart",
...
@pytest.mark.asyncio
async def test_restart_after_decision_commit_creates_exactly_one_repair_order(
    engine: AsyncEngine,
    ...
    try:
        ...
        approval = await ApprovalRepository(engine).submit_bound_once(
            BoundApprovalCommand(
                ...
                approver_id="maintenance.restart.manager",
                decision=ApprovalDecision.APPROVED,
                reason="continue the bound repair plan after restart",
        ...
        assert detail.work_order.payload["kind"] == "repair"
