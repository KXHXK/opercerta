...
2. 只实施 OperCerta；按库存补货纵向闭环计划从 Task 8 FastAPI 创建、查询与绑定审批边界开始，不跨 Task 抢跑。
...
> 工作目录为本 OperCerta 仓库根目录。请先读取 `DOCUMENT_INDEX.md`、`docs/development-log/current-state.md`、最近每日日志、`README.md`、`IMPLEMENTATION_HANDOFF.md`、`docs/specs/` 下的四份设计文件、库存补货纵向闭环设计及其实施计划；库存补货 Task 1–7 已完成本地门禁，下一步从计划 Task 8 的 FastAPI 创建、查询与绑定审批 RED 继续。严格只实施 OperCerta，不复用旧公司材料，不虚构指标，未通过发布门禁前不启动其他项目。
