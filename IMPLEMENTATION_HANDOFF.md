...
- Task 4 已确认采用通用 JSON object payload，并固定新工单初始状态为 `created`；书面规格位于 `docs/superpowers/specs/2026-07-16-work-order-idempotency-contract-design.md`，当前待用户复核，尚未写生产代码。
...
2. 只实施 OperCerta；先复核 Task 4 幂等工单书面规格，确认后回填实施计划，再开始幂等写入 TDD。
