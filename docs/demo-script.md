...
7. 展示真实模型证据摘要：三业务 6 个代表 operation、3 条真实模型路径；强调模型只解释，且原文/token/费用没有被虚构记录。
8. 如面试官追问工程排障，从 `/engineering` 的事故复盘选择 1--2 个案例，按“观察—根因—修复—验证—限制”讲述，不逐项念页面。
9. 结束时明确边界：真实模型只完成本地代表性运行；生产 IAM/SSO、公开交互 HTTPS 后端、自动部署和 Release Tag 尚未完成，公开专题不提供可写服务。
