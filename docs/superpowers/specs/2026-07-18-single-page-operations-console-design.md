...
| 右侧时间线 | SSE 审计事件 | 首次读取全量快照，随后用最后 sequence 作为 `Last-Event-ID` 请求续传；按 sequence 去重排序。 |

用户可输入 operation ID 读取既有演示操作。错误按已有稳定 API envelope 显示中文提示：401、403、404、409、422、503 均不显示内部字符串。

## 4. 数据流

1. 用户选择演示角色，向 demo-token 端点换取内存 token。
...
4. 前端在每次详情读取后请求 SSE 快照，将已见最大 sequence 传为 `Last-Event-ID`；客户端仅追加更大 sequence。
5. 后端返回终态、工单或安全错误；前端渲染结构化字段，不解释或推断隐藏业务状态。

## 5. 测试与验收

- 前端单元测试覆盖角色可见性、创建请求、六字段审批绑定、稳定错误映射和 sequence 去重。
