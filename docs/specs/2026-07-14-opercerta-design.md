# OperCerta｜智能运营处置 Agent：详细设计

**文档状态：** 书面总审通过，作为后续实施基线  
**设计日期：** 2026-07-14  
**项目阶段：** 第一个顺序实施项目；发布复盘后再开始 ForenTrail
**核心框架：** LangGraph  
...
OperCerta 将运营人员的自然语言查询或处置请求转换为有证据、可校验、可审批、可恢复的业务流程，并在批准后创建幂等的模拟工单，最终返回可审计结果。
...
传统页面要求用户知道对象编号、规则入口和操作顺序；直接使用聊天模型又容易缺少证据、臆造状态或绕过审批。OperCerta 在两者之间增加一个受控 Agent 层：模型理解语言并组织说明，确定性服务掌握业务事实、风险和执行权限。
...
新的实施对话应读取本规格、总体设计、组合设计和 IMPLEMENTATION_HANDOFF.md，然后重新核对依赖版本，基于新制定的 OperCerta 实施计划按 TDD 开发。实施过程中若发现设计需要修改，应先回写 ADR/规格再编码；不得用代码悄悄改变业务边界。
